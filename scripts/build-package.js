 ...
 * - yarn build
 * - @react-native/codegen yarn build (not part of monorepo yarn build)
 * - yarn build-types
...
async function buildPackage() {
  ...
  if (
    exec('yarn build', {
      cwd: path.join(PACKAGES_DIR, 'react-native-codegen'),
    }).code !== 0
  ) {
    process.exit(1);
  }
...
function cleanupStaleCodegenArtifacts() {
  // git reset --hard HEAD doesn't delete stale files in gitignored directories.
  // A leftover codegen lib/ can keep requiring removed deps (e.g. flow-parser);
  // leftover FBReactNativeSpecJSI-generated.cpp can fail native builds.
  const staleCodegenPaths = [
    path.join(PACKAGES_DIR, 'react-native-codegen', 'lib'),
    path.join(REACT_NATIVE_PACKAGE_DIR, 'React/FBReactNativeSpec'),
    path.join(
      REACT_NATIVE_PACKAGE_DIR,
      'ReactAndroid/build/generated/source/codegen',
    ),
  ];

  for (const staleCodegenPath of staleCodegenPaths) {
    fs.rmSync(staleCodegenPath, {recursive: true, force: true});
  }
}
